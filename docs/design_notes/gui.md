...
Known blind spot, PRESERVED rather than fixed: the lookup is `group_bc[patch_name]`, but a `group_bc` key is the per-segment grouping LABEL while a `.bnd` patch name is the physical BC TYPE the mesher resolved that label to (`src/Mesh.cpp`, `Config::resolveGroupBc` — the same two namespaces the mesh-BC audit confused once, and had to stop comparing directly). The precedence rule therefore only fires where a label happens to be named after its own type, which is the common case and not the general one. #90 is a prefactor and changes no behaviour; the asymmetry is recorded here and in the service's own docstring so that whoever picks up #91/#92 meets it before rediscovering it.
