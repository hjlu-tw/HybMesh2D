...
16. **逐邊 wall BC 記錄** — 趁邊界順序已知，把 `boundaryEdgeBc[{min,max}]` 與 `boundaryEdgeSeg[...]` 依「邊 i 屬於起點節點所在段」寫入 — `:794-810`
...
- **D. 後處理與交棒**（`:1248-1391`）— 平行四邊形群組橫向平衡 :1251-1298 → **slide junction 的 BC 依弧長逐邊承接**（寫入 `boundaryEdgeBc` :1355 與 `boundaryEdgeSeg` :1356）→ **遠場內邊界邊發射**（`m_mesh.addEdge` :1370，並把 `bcTag`/`segKey` 掛到 `edges.back()` :1381-1388）→ `return lastH` :1391
...
- 公開表 `boundaryEdgeBc`(:82，key 為排序後的節點對) 與 `boundaryEdgeSeg`(:87)
...
6. **建立 worker 與接線**：`MeshGenWorker(exe, tmp_cfg.name)`(219) → `log_signal → main_window.log_panel.log`(220) → `progress_signal → _on_mesh_gen_progress`(221 → 282-283 → `main_window.set_progress("mesh", pct)`) → `finished_signal → lambda rc: _on_mesh_gen_finished(rc, tmp_cfg.name, expected_vtk)`(222-224) → `main_window.claim_progress("mesh", determinate=True)`(227) → `start()`(228)。
