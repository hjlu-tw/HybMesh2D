...
- **`CLAUDE.md`** at the repo root — this repo keeps an unusually dense body of hard-won invariants there (the BL/no-BL junction rules, the panel↔model data-flow contract, the length-unit/`Linf` relationship, the pop-up stacking rules). Treat it as required reading alongside `CONTEXT.md`, not as an alternative to it.
