...
miss = cfg5.missing_geometry_files()
check(miss == [gone],
      f"6. missing_geometry_files() names exactly the entry that is not on disk "
      f"(got {miss})")
check(not MeshConfig().missing_geometry_files()
      and not cfg6_ok(absolute).missing_geometry_files(),
