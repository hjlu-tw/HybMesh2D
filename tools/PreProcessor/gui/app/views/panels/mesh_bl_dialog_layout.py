        ...
        for title, start_expanded, hint, keys in groups:
            ...
            for key in keys:
                ...
                if not reads_in_mode(spec, self._mesh_mode):
                    # BOTH, and `cell` is `w` for every row but one. A field that can
                    # be greyed with a reason rides in a composite cell
                    # (`_field_cell`): hiding `w` alone leaves that cell -- and the
                    # note in it -- showing beside nothing, while hiding only the cell
                    # leaves `w.isHidden()` False, because Qt does not set a child's
                    # own hidden flag when a parent goes invisible. The gate asks the
                    # WIDGET (`test_field_spec_tables.py` check 14h), and the honest
                    # answer is to hide it rather than to lean on its parent.
                    w.setVisible(False)
