...
from PyQt6.QtCore import QEvent, QObject, Qt, QPoint, QTimer
...
def keep_on_top(widget: QWidget) -> QWidget:
    ...
    * ``Qt.Tool`` fixes that, but on macOS a Tool window is an NSPanel with
    ...
    app is in front, and coverable by it — and _PopupRaiser lifts it back above
    the main window on activation. Parenting to the TOP-LEVEL window is what
    makes both halves work: the raiser finds the pop-up in that window's child
    list, and a pop-up parented to a panel/canvas/sidebar would be hidden along
    with that panel when the selection changes (mirrors the explicit
