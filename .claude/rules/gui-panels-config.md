...
Loaded on demand when a panel view, the physical-length spin box, a field-spec
service, the unit service or a mesh-config model is read. Rules only: the
rationale — the counted attributes, the measurements, the injections, the
reversals and the named blind spots — is `docs/design_notes/gui.md`. Read that
note before overruling a rule here, and when a rule changes update BOTH.
