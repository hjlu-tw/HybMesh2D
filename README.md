...
| `BL_JUNCTION_ANGLE_C2` | （度）θ ≤ C2 → 沿 BL 法向垂直封蓋 | 270.0 |
...
> **C1 為何仍在？** 凹交界（θ ≤ C1）原本是「情形 1」：BL 列沿 no-BL 鄰邊**滑行**並吸收該段的表面節點。在內流的凹角（封閉流域向內長 BL，最常見的情況）這會把邊界層塌陷到使用者明確標成 no-BL 的那面牆上，而且不保持層高。因此凹交界現在同樣採**垂直封蓋**；`C1` 僅保留給 `BL_JUNCTION_METHOD=0` 與設定檔往返，調整它不會改變預設方案的行為。
