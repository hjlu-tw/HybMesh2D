...
### 8. BL / no-BL 交界處理 (BL/no-BL Junction)

「交界」指一個生長邊界層的節點，其相鄰段被標記為**不生長 BL**（`.meta` 的 `grow=0`，即 GUI 網格階段的逐段 **No-BL**）。此處若沿用一般的角平分線方向，成長射線會被拉向 no-BL 邊，使該格歪斜甚至反轉。

| 參數名稱 | 說明 | 預設值 |
| :--- | :--- | :--- |
| `BL_JUNCTION_METHOD` | 0: 逐漸收斂至零（舊行為）；1: 角度驅動封蓋 | 1 |
| `BL_JUNCTION_ANGLE_C1` | （度）僅 method 0 與設定往返時保留，見下方說明 | 135.0 |
| `BL_JUNCTION_ANGLE_C2` | （度）95 < θ ≤ C2 → 沿 BL 法向垂直封蓋 | 270.0 |
| `BL_JUNCTION_ANGLE_C3` | （度）C2 < θ ≤ C3 → 沿鄰邊反向延伸封蓋；θ > C3 → 垂直封蓋 | 315.0 |

**θ 的定義**：由 BL 邊掃向 no-BL 邊、**穿過流體側**的夾角。180° = 共線，< 180° 為凹，> 180° 為凸。因為以流體側定義，內流與外流用同一套判斷，不需分別設定。

**method 1（預設，角度驅動封蓋）**

| θ 區間 | 成長方向 |
| :--- | :--- |
| θ ≤ 95° | 沿 no-BL 鄰邊**滑行**，並吸收被覆蓋的 no-BL 節點 |
| 95° < θ ≤ C2 | 沿該 BL 邊自身的外法向**垂直封蓋** |
| C2 < θ ≤ C3 | 沿 no-BL 鄰邊的**反向延伸**封蓋 |
| θ > C3 | 垂直封蓋 |
| 兩側鄰居皆為 no-BL（孤立 BL 節點） | 垂直封蓋 |

此方案**不做高度收斂**。跨所有情形保持不變的是邊界層的**垂直總高度** `D_total`：傾斜的封蓋若以固定「邊長」成長，只會達到 `D_total × cos(傾角)` 而低於旁邊的垂直列、把角落拉歪，因此步長以 `1 / cos(傾角)` 補正（分母設下限，避免極銳的凹角把列長放大到失控）。封蓋會留下一根**全高的側向列**，其外露的側邊會輸出為遠場約束邊，使該楔形由三角形填補、而非用四邊形硬蓋過去。

> **θ ≤ 95° 為何改為滑行？這個界線為何不是參數？** 封蓋必須指進角落的**流體楔形**內。楔形的張角就是 θ，而垂直封蓋固定在離 BL 邊 90° 的方向——因此 θ ≤ 90° 時垂直封蓋會指到 no-BL 牆上或更外側，整根列直接走出流域：θ < 90° 時最終前緣會與 no-BL 表面相交（`HYBMESH_ERROR 5 BL`，回報 self-intersection）；θ 恰為 90°（也就是「矩形風道其中一面標成 No-BL」這種最普通的情況）封蓋剛好落在牆上，前緣自我折返，Gmsh 收到退化的孔洞邊界（`HYBMESH_ERROR 6 GMSH`）。這種交界只能倚靠鄰邊——是楔形太窄放不下封蓋，不是邊界層太厚。略大於 90° 時封蓋雖然合法，卻會沿整根列留下一條細長楔形，故再留一小段保護帶一併滑行。這是幾何上的界線而非偏好，所以固定為 95°，不從 `C1` 讀取。
>
> **C1 為何仍在？** `C1` 原本就是這條界線，預設 135°——寬到連足以容納正常垂直封蓋的角落也一併滑行，把邊界層無謂地塌陷到使用者標成 no-BL 的牆上。`C1` 現僅保留給 `BL_JUNCTION_METHOD=0` 與設定檔往返，調整它不會改變預設方案的行為。

**極銳楔形（`[WARN] Very sharp BL/no-BL wedge at (x, y)`）**：滑行成立的前提是凹角混成能把被壓縮的列扳過去。楔形以 θ 收斂，因此角落壓縮掉 `D_total / tan θ` 長度的壁面，而混成只能覆蓋 `BL_CONCAVE_INFLUENCE_MULTIPLIER × D_total`——臨界條件是 `tan θ × influence ≈ 1`，**與邊界層厚度無關**。預設 influence 2.5 對應 21.8°，實測也正是 22°（可生成）與 21°（失敗）之間；influence 改 5.0 臨界移到 15°/10° 之間，改 1.5 則移到 35°/30° 之間。超過臨界後，錯誤要到很後面才以「前緣自相交」或 Gmsh 失敗浮現，而訊息會指向整個模型的 BL 尺寸，因此交界處現在會**事先發出警告並指名該角落座標**。這只是提醒，不會自動修正。處理方式：在該處降低 BL 高度（可用逐幾何覆寫）、讓鄰段也長 BL、把角落開大，或依警告所給的數值調高 influence multiplier。

**method 0（舊行為，逐漸收斂至零）**：交界節點沿自身 BL 邊的法向成長（而非角平分線），同時每個節點的層高乘上一個收斂係數——在交界處約為 12%，並沿弧長距離平滑回到 1，使外緣朝交界處下降而沒有斷崖，收縮出的楔形交由遠場網格填補。

GUI 可在 **Mesh 面板 → Concave 區塊**，或 **Edit BL** 對話框調整這四個參數。
