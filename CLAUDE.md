...
- **Order by ITERATION COUNT, and lineage is NOT recoverable — which is why.**
  The issue asks to order by `RUN.txt`'s "recorded lineage"; measuring what the
  note holds says that cannot be done, since `resumed_from` is a BASENAME and
  every leg's dump is the same solver output under the same name, so normalising
  the two spellings collapses every leg onto one key. `last_iteration` is what
  the note does hold per leg, so that is the axis, with creation order as the
  tie-break.
...
- **An overlap is REPORTED, never interleaved.** #31 makes re-running a leg from
  an earlier point a click, so a later run can cover ground an earlier one did.
  Since the per-leg START iteration cannot be recovered, the overlap is inferred
  from NON-MONOTONICITY (a leg that ran later reporting no higher a count) and
  said out loud with both legs named; nothing is merged or spliced.
...
- **The leg name prefixes a label only when the series has more than one file**
  ...
  it did. `set_result`'s triangulation reuse and #24's clim precedence
  (manual > lock > auto) are unchanged and both are pinned across a leg boundary.
Gated by `tests/test_result_legs_playback.py` — 13 properties over 3 groups,
every one verified by injection, with its two blind spots and the acceptance run
...
- **`services/restart_points.py`** (Qt-free) + **`views/panels/restart_chooser.py`**: **the restart point is PICKED from the case's own history, not typed as a path.** USER-REQUESTED (2026-08-21, issue #31), blocked by #30 because it reads the `RUN.txt` #30 writes. After restarting once the next run is one of two intentions — **continue further** from the newest dump, or **re-run the same leg** from the dump the *last* run resumed from, having looked at the results and wanted that segment redone — and a `Restart` tick plus a free-text `zdump_fn_restart` expressed neither. The autofill (`_autofill_restart_from_last_run`, now gone) looked for a fixed `binDumpZ.dat` + `.gui`/`.cli` **in `work/` only**, so it knew nothing about the `work/prev_<NNN>/` archives #26 creates: "re-run the same leg" meant the user remembering which file that was and browsing to it, while the thing actually being decided is an **iteration count**. `list_restart_points(case_root)` now returns the rows — cold start, the newest un-archived dump in `work/`, then each archived leg newest-first with the iteration count, timestamp and run tag from its own `RUN.txt` — and the chooser is one column of radio buttons over them plus an "Other file…" escape. Rules that are load bearing:
  ...
  - **An archive with no `RUN.txt` still gets a row**, with the count shown as unknown; its convergence history is deliberately not re-read (the note IS the record, and a second way to compute the field would be a second answer). And the count is rendered as a **bound** — `1990+`, with the tooltip explaining — because `RUN.txt` records the last convergence ROW and the run reached at least that (#30, measured 1990 vs the solver's 2000).
  ...
  - **Two departures from the issue's own text, both deliberate.** The rows show the count as a **bound** (`1990+`) where the issue's mock shows a bare `iteration 2000`: `RUN.txt` records the last convergence ROW and #30 measured that a run reaching 2000 leaves 1990 there, so printing the spec's number would be a fabrication — the tooltip carries the explanation. And the issue says this keeps "`prepare_case_dir` untouched", which it does not: `bare_link_for_archived_dump` is called from it, because without the link the chooser's headline click is unrunnable (test check 11 was RED before it existed).
