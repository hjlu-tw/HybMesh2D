...
observed. The cost is therefore not truncation but **always-loaded context** — **38,091
characters (38,311 bytes, 2026-09-24) ≈ 10k tokens**, paid by every session before it reads a line of
...
**39,000**, leaving 909 characters of slack — derived by a rule the gate states at that
...
this sentence; and since #109 a check there READS all three of that statement's numbers — the
band's two ends and the boundary — and fails when the slack leaves the band, naming a budget value
that would put it back. Re-banding stays a decision, so `--sync` does not touch the constant. #75 had locked it at 40,000 while this
...
characters while `wc -c` reports bytes, and the two differ by 220 today because of the CJK in this
...
| GUI panel configuration (the field-spec tables, the one-directional panel↔model data flow, the derived `.dat` key map, the Edit-BL dialog's grouping, length units and `Linf`, the physical-length spin box, the geometry list's file identity) | `.claude/rules/gui-panels-config.md` | `views/panels/**`, `views/clean_double_spin_box.py`, the field-spec / units / config-ownership services, `models/mesh_config*` — the exact globs are that file's own `paths:` list, and its header names the controllers it also governs from outside them. Two of its globs are WIDER than the area: a results panel's rules are in `gui-results.md`, `views/panels/restart_chooser.py`'s are in `pipeline-case.md`, and `MeshConfig.output_base`'s Output-`.*` rule is in `gui-handoff.md` with `models/mesh_output_names.py`, where #77 moved it — as is `views/panels/mesh_stats_panel.py`'s quality-summary rule, that file being matched by both (#131). **The TOPOLOGY template library left this file in #137**, with the mesh-canvas globs #136 had added for its overlay; the row below has it. |
| GUI topology templates (a family as a pure function, the parameter declarations, the projection to JSON, binding to a CAD segment by STABLE ID, the refusal when one breaks and the panel that repairs it, the block skeleton drawn on the mesh canvas) | `.claude/rules/gui-topology.md` | `services/topology_*`, `views/mesh_canvas*`, `views/panels/mesh_config_*`, `models/mesh_config_io*` — the exact globs are that file's own `paths:` list, and its header names the controllers it also governs from outside them. The ELEVENTH rule file and the second taken because one was FULL (#85 was the first): #137's review left `gui-panels-config.md` with 73 characters of slack. Its last two globs are that file's as well, the intended overlap — a session editing the mesh-config panel or the mesher-config writer loads BOTH, the field-spec mechanism from there and the template's own rules from here. |
| GUI canvas and editing (the owner of the edge being edited, the outline re-fit, global undo, duplicate/transform closure, the one-polyline discrete geometry, the parametric NACA aerofoil and the shape-parameter tables, pop-up stacking) | `.claude/rules/gui-canvas-edit.md` | `views/canvas*`, `services/edge_edit*`, `services/shape_refit*`, `services/naca_airfoil*`, `services/canvas_tools*`, `models/shape_spec*`, `commands/**`, `app/popup_stack.py` — the exact globs are that file's own `paths:` list, and its header names the controllers, views and dialogs it also governs from outside them — pop-up stacking reaches every module that shows a modeless pop-up, none of them under a glob of that file, and the header carries the count so this row cannot disagree with it. One glob is WIDER than the area: `commands/config_cmds.py`'s other half is in `gui-panels-config.md`. |
...
**Eleven rule files now, and NO area of residue is left.** #59 planned six, derived from this
...
the one they state about the TREE (#101) — measured against disk, and the root budget against
the band rule stated at its own definition (#109).
...
- **GUI file length.** Keep each file under `tools/PreProcessor/gui/` at **~500 lines**; split it
  ...
  **four** commits crossed it in the six days after #67 wrote it down — `98003f1` (#85),
  ...
  history. 5 of 277 files exceed it (worst 524); those five are PINNED at their measured sizes,
...
- **`golden_mesh.py`**: `capture <dir>` / `compare <dir>` over 19 mesher cases (~8 s), for proving
  ...
  the 1e-10 tolerance — and a case that produces NO mesh (one junction shape is expected to refuse)
  reports `NO MESH`, never a deviation measured off nothing, so an `N/N SAME` cannot be quoted as N
  meshes (`tests/test_golden_comparator.py`). It compares the `.vtk` AND both STAR-CD files, because the `.cel` is the grid
...
landed in `68d3945^..23bbe34`, which the document could not know about.
