...
observed. The cost is therefore not truncation but **always-loaded context** — **82,874
characters (83,344 bytes, 2026-09-01, after #63 moved the pipeline and solver-case rules out to
`.claude/rules/pipeline-case.md`) ≈ 21k tokens**, paid by every session before it reads a line
...
`wc -c` reports bytes, and the two differ by 470 today because of the CJK in this repo's own
